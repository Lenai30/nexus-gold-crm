...
import { Users, TrendingUp, Clock, Target, Flame, ArrowUpRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: "Em Negociação", value: leads.filter((l) => l.column === "negociacao").length, icon: TrendingUp, change: "+8%" },
    { label: "Follow-ups Hoje", value: leads.filter((l) => l.column === "followup").length, icon: Clock, change: "3 pendentes" },
    ...
    <div>
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        ...
        <div className="space-y-3">
          {COLUMNS.map((col) => {
            const count = leads.filter((l) => l.column === col.id).length;
