...
import { Lock, X, TrendingUp, Clock, Target, DollarSign } from "lucide-react";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from "recharts";
import { motion, AnimatePresence } from "framer-motion";
import { COLUMNS } from "@/data/leads";
...
export default function Relatorios() {
  ...
  const chartData = COLUMNS.map((col) => ({
    name: col.title,
    leads: leads.filter((l) => l.column === col.id).length,
  }));
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        ...
        >
          ...
          <input
            ...
            className={`w-full px-4 py-2.5 rounded-lg bg-muted border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-gold transition-all mb-3 ${
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg gradient-gold text-primary-foreground font-semibold text-sm hover:opacity-90 transition-opacity"
          >
            Desbloquear
          </button>
        </motion.div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData}>
            ...
            <Bar dataKey="leads" radius={[6, 6, 0, 0]}>
              {chartData.map((_, i) => (
                <Cell key={i} fill={`hsl(43, 74%, ${49 + i * 5}%)`} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
