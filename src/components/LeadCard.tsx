...
export default function LeadCard({ lead }: { lead: Lead }) {
  ...
  return (
    <motion.div
      ...
      className="bg-card border border-border rounded-xl p-4 shadow-sm hover:shadow-gold/10 hover:border-gold/30 transition-all duration-300 cursor-grab active:cursor-grabbing"
    ...
    >
      ...
      {(lead.campanha_nome || lead.conjunto_nome || lead.anuncio_nome) && (
        <div className="mb-3 p-2 rounded-lg bg-gold/5 border border-gold/20">
      ...
      <div className="flex gap-2 pt-2 border-t border-border">
        ...
        <button onClick={(e)=>{e.stopPropagation(); if(confirm("Excluir lead?")) deleteLead(lead.id);}}
          className="flex items-center justify-center w-9 rounded-lg text-xs bg-danger/10 text-danger hover:bg-danger/20 transition-colors">
